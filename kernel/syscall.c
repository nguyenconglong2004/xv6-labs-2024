...
// Array of system call names, indexed by syscall number
...
void syscall(void)
{
  ...
  if (num > 0 && num < NELEM(syscalls) && syscalls[num])
  {
    ...
    uint64 trace_proc = 1 << num;
    int retval = p->trapframe->a0;
    if (p->trace_mask && ((p->trace_mask & trace_proc) != 0))
    {
      printf("%d: %s -> %d\n", p->pid, syscalls_name[num], retval);
    }
  }
  else
  {
